...
function ($, _, d3, moment) {
	...
	// Setup navigation
	// Due to the relative young age of Ghost, there is no advanced tools
	// to make a fully HTMl navigation
	$("nav li").removeClass("active").find("a[href='" + url + "']").parent().addClass("active");
	...
	if($("body").hasClass("home-template")) {
		...
		var scrollAnimate = function() {
			var s = $(window).scrollTop();
			$("#splash").fadeTo( 0, .9 - (s/height));
			$(".home-content").fadeTo( 0, (s/height) * .5);
	...
	if($("body").hasClass("page-template-resume")) {
		...
		function drawDonutChart(element, label, percent, val) {
